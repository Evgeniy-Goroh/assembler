;Программа выводит на экран список файлов из корневого каталога. При нажатии клавиши «S» программа сортирует этот список по алфавиту
        model small
        .386
		stack 100h
;--------------------------------------------------------------------------------------------------------
        dataseg
sector                  db      512 dup (0)   	;копия сектора диска
off_in_sector           dw      0      	        ;смещение в прочитанном секторе
titles                  db      2688 dup (0)    ;массив имен файлов
title_copy              db      15 dup (0)      ;копия имени файла
...
copy_si					dw		0				;копия регистра si						
copy_di					dw		0				;копия регистра di
multipl					db		12				;множитель для рассчета смещения
...
;очистка экрана
    ...
    mov cx, 0000h   ;верхняя левая ячейка
    mov dx, 184fh   ;нижняя правая ячейка
    int 10h
;--------------------------------------------------------------------------------------------------------
    lea si, titles  ;загружаем адрес массива имен (имя = 12 байт)
...
lp:
    ...
    jmp lp
;----------------------
;копируем имена файлов из записей, содержащихся в текущем секторе
choose:
    push ax
    push bx
    push cx
    ...
    mov cx, 16
    ;по всем 16 записям, содержащимся в текущем секторе
	m:
        ;подсчет смещения в sector
        mov ax, 16
        sub ax, cx
        shl ax, 5
		...
		;проверяем: обычный ли файл, или нет?
        mov al, byte ptr [bx]
        cmp al, 0               ;запись пуста
        je Not_file
        cmp al, 0E5h            ;файл был удален
        je Not_file
        cmp al, 2Eh             ;файл является поддиректорией
        je Not_file
